...
import type { AdvancedLessonDefinition, AdvancedLessonRecord } from "@/lib/learning-advanced";
import {
  addAdvancedEngagedSeconds,
  advancedLessonVideoId,
  loadAdvancedProgress,
  upsertAdvancedLesson,
} from "@/lib/learning-advanced";
import { formatDateTime, formatDurationSeconds } from "@/lib/learning-basics";

type PlayProps = {
  lesson: AdvancedLessonDefinition | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSaved: () => void;
};

export function AdvancedPlayDialog({ lesson, open, onOpenChange, onSaved }: PlayProps) {
  const [livePct, setLivePct] = useState(0);
  const sessionStartRef = useRef(0);

  const seekSeconds = useMemo(() => {
    if (!open || !lesson) return 0;
    const p = loadAdvancedProgress()[lesson.slug]?.watchPercent ?? 0;
  ...
  useEffect(() => {
    ...
    const rec = loadAdvancedProgress()[lesson.slug];
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (sec > 0) addAdvancedEngagedSeconds(lesson.slug, sec);
      onSaved();
    };
  }, [open, lesson, onSaved]);
  ...
  const onProgress = useCallback(
    (pct: number) => {
      ...
      upsertAdvancedLesson(lesson.slug, { watchPercent: pct });
    },
    [lesson]
  );
  ...
  const onEnded = useCallback(() => {
    if (!lesson) return;
    upsertAdvancedLesson(lesson.slug, { watchPercent: 100 });
    setLivePct(100);
    onSaved();
  }, [lesson, onSaved]);
      ...
      <DialogContent className="max-h-[95vh] max-w-3xl overflow-y-auto">
        ...
        <div className="space-y-4">
          <YoutubeChapterPlayer
            ...
            videoId={advancedLessonVideoId(lesson)}
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Reaches 100% when the video finishes. Progress is saved on this device.
            </p>
          </div>
        </div>
      </DialogContent>
...
export function AdvancedReviewDialog({ lesson, open, onOpenChange }: ReviewProps) {
  const [record, setRecord] = useState<AdvancedLessonRecord | null>(null);

  useEffect(() => {
    if (!open || !lesson) return;
    setRecord(loadAdvancedProgress()[lesson.slug] ?? null);
  }, [open, lesson]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Progress summary from this device</DialogDescription>
        </DialogHeader>
