...
export default function HoldingsPage() {
  ...
  const { data, isLoading } = useSWR("/api/portfolio/overview", fetcher, {
  ...
  // 4. Extract holdings from the API response safely
  // We map over the data to ensure defaults (0) for any missing numbers to prevent crashes
  const rawHoldings = data?.holdings || [];
  
  const holdings = rawHoldings.map((h: any) => ({
    symbol: h.symbol || "UNKNOWN",
    name: h.name || h.symbol, // Fallback name
    sector: h.sector || "N/A",
    shares: h.quantity || 0, // API might call it 'quantity', your UI used 'shares'
    avgCost: h.averagePrice || 0,
    currentPrice: h.currentPrice || 0,
    marketValue: (h.quantity || 0) * (h.currentPrice || 0),
    gainLoss: h.unrealizedPnL || 0,
    gainLossPercent: h.unrealizedPnLPercent || 0,
    dayChange: h.dayChange || 0,
    dayChangePercent: h.dayChangePercent || 0,
    weight: h.allocationPercent || 0, // Assuming API sends allocation
  }));

  // Calculate summary stats dynamically
  const totalValue = holdings.reduce((acc: number, curr: any) => acc + curr.marketValue, 0);
  const totalDayChange = holdings.reduce((acc: number, curr: any) => acc + (curr.dayChange * curr.shares), 0);
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
          ...
          <div className="grid gap-4 md:grid-cols-4">
            ...
            {/* Note: 'Best Performer' logic would require sorting the holdings array */}
          </div>
                  ...
                  {holdings.length === 0 ? (
                    ...
                    holdings.map((holding: any) => (
                      <TableRow key={holding.symbol} className="hover:bg-muted/50">
                        ...
                        <TableCell>${holding.avgCost.toFixed(2)}</TableCell>
                        <TableCell>${holding.currentPrice.toFixed(2)}</TableCell>
                        <TableCell className="font-medium">${holding.marketValue.toLocaleString(undefined, { minimumFractionDigits: 2 })}</TableCell>
                        <TableCell>
                          <div className={holding.gainLoss > 0 ? "text-green-600" : "text-red-600"}>
                            <div className="font-medium">
                              {holding.gainLoss > 0 ? "+" : ""}${Math.abs(holding.gainLoss).toFixed(2)}
